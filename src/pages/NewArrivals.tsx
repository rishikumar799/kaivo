...
export default function NewArrivals() {
  ...
  // Filtering products where newArrival is strictly TRUE
  const filteredProducts = useMemo(() => {
    let result = db.products.filter((p) => p.newArrival);
    ...
    return result;
  }, [db.products, selectedCategories, selectedSizes, selectedColors, priceRange, sortBy]);
  ...
  const sizeCountMap = useMemo(() => {
    ...
    db.products.filter(p => p.newArrival).forEach((p) => {
      p.sizes.forEach((s) => {
        map[s] = (map[s] || 0) + 1;
      });
    });
  ...
  return (
    <div className="bg-black text-white py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen">
      ...
      <div className="border-b border-zinc-900 pb-8 mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <span className="text-[10px] font-mono tracking-[0.3em] text-amber-500 uppercase block mb-2">NEW DROP</span>
          ...
          <p className="text-xs text-zinc-500 font-sans mt-2 max-w-lg">
            Fresh styles. Premium comfort. Made to stand out. Explore our latest heavyweight drops that redefine relaxed styling.
          </p>
        </div>
      ...
      <div className="flex gap-10">
        ...
        <div className="flex-grow flex flex-col justify-between">
          <div className="flex items-center justify-between mb-6 text-xs text-zinc-500 font-mono">
            <span>NEW ARRIVALS ({filteredProducts.length})</span>
          </div>
          ...
          {filteredProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center border border-zinc-900 bg-zinc-950/20 p-8 rounded-sm">
              <p className="text-zinc-500 font-mono text-sm uppercase tracking-widest mb-4">No drops matching filters</p>
              <button
                onClick={clearAllFilters}
